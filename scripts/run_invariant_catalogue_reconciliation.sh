...
# when reconciliation fails — the same shape as run_work_catalogue_reconciliation.sh and run_meter_list_reconciliation.sh,
...
lock_file="${DOCPLANE_INVARIANT_CATALOGUE_LOCK_FILE:-/run/lock/docplane-invariant-catalogue.lock}"
metrics_file="${DOCPLANE_INVARIANT_CATALOGUE_METRICS_FILE:-/var/lib/node_exporter/textfile_collector/docplane_invariant_catalogue.prom}"
...
flock -n -E "$FLOCK_CONFLICT_EXIT" "$lock_file" \
  python3 "$repository_root/scripts/invariant_catalogue.py" "$@"
reconcile_status=$?
