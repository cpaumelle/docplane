...
cleanup() {
  ...
  rm -f branding/brand.css branding/logo.svg branding/favicon.svg "$ENV_FILE"
}
trap cleanup EXIT
...
log "2. site-init owns the site volume"
compose ps -a --format '{{.Service}} {{.State}}' | grep -E '^site-init' || fail "site-init missing"
OWNER=$(compose exec -T docs-api sh -c 'stat -c "%u:%g" /data/site')
[ "$OWNER" = "10001:10001" ] || fail "docs-site owned by $OWNER"
compose exec -T docs-api sh -c 'test -w /data/site && test -f /data/site/.docplane-site-init' || fail "site volume not ready"

log "3. bootstrap contributor and publish first page"
...
[ "$DEPLOY" = "COMPLETED" ] || fail "first publication failed"

log "4. certification and configured site identity"
...
printf '%s' "$HTML" | grep -q "Synthetic fresh-install probe" || fail "page missing"
printf '%s' "$HTML" | grep -q "Integration DocPlane" || fail "DOCPLANE_SITE_NAME did not reach MkDocs"

log "5. routed human and agent surfaces"
curl -fsS "$FRONT/dashboard/" >/dev/null || fail "dashboard route failed"
curl -fsS "$FRONT/assets/app.js" >/dev/null || fail "dashboard asset failed"
curl -fsS "$FRONT/.well-known/docplane.json" | grep -q 'docplane-agent-discovery-v2' || fail "discovery route failed"
curl -fsS "$FRONT/openapi.json" | grep -q '"title":"DocPlane"' || fail "OpenAPI route failed"
curl -fsS "$FRONT/healthz" | grep -q '"page_counts"' || fail "authority health route failed"
NOAUTH=$(curl -s -o /dev/null -w '%{http_code}' "$FRONT/api/v1/capabilities")
[ "$NOAUTH" = "401" ] || fail "unauthenticated API returned $NOAUTH"
curl -fsS "${AUTH[@]}" "$FRONT/api/v1/pages?limit=1" | grep -q '"total"' || fail "authenticated routed API failed"

log "6. neutral assets and local overlay"
BASE="$FRONT/assets"
...
curl -fsS "$BASE/brand.css" | grep -q "brand-override slot" || fail "fallback did not resume"

log "7. generated-site ownership invariants"
...
log "9. routed publication retry"
...
[ "$RETRY" = "COMPLETED" ] || fail "routed retry returned $RETRY"
