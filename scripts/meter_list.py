...
def _rule_section(rule: dict[str, Any], group_name: str | None = None) -> list[str]:
    ...
    # The heading text is the bare rule name, so the site slug (#<lower-cased name>)
    # is the rule's stable anchor inside the single catalogue page.
    lines = [f"### `{rule['name']}`", "", f"{rule['rule_kind']}{group}{watches}{severity}{pending}", ""]
    if rule.get("summary"):
...
def catalogue_path(source_key: str) -> str:
    """The one generated page for a monitoring source: every rule is a record
    inside it (record cardinality, not page cardinality)."""
    return f"{SECTION}/{_path_slug(source_key)}/index.md"


def render_pages(source_key: str, structure: dict[str, Any], structure_hash: str) -> list[dict[str, str]]:
    """One catalogue page per monitoring source (1.5.0; 1.4.0 emitted one page per
    rule file). Rule files are `##` sections headed by the file stem and rules are
    `###` headings holding the bare rule name, so both anchors are stable slugs:
    #<file-stem> and #<lower-cased rule name>. Retired per-file paths are archived
    and redirected here by the publishing change."""
    stamp = (
    ...
    # Two stems that slug alike would share one section anchor; refuse, as the
    # per-file layout refused two files fighting over one page.
    stem_slugs: dict[str, str] = {}
    for file_stem in sorted(structure):
        slug = _path_slug(file_stem)
        if slug in stem_slugs:
            raise RuntimeError(
                f"rule files {stem_slugs[slug]!r} and {file_stem!r} both slug to anchor {slug!r} — rename one"
            )
    ...
    body = [
        ...
        *LIFECYCLE_LINES,
        ...
        stamp,
        "",
        f"{total_rules} rules across {len(structure)} rule files. Each rule is anchored by its "
        "lower-cased name (for example `#backupstale`); each rule file by its stem.",
        "",
    ]
    for file_stem in sorted(structure):
        rule_count = sum(len(rules) for rules in structure[file_stem].values())
        body.append(f"- [`{file_stem}`](#{_path_slug(file_stem)}) — {rule_count} rules")
    body.append("")
    for file_stem in sorted(structure):
        groups = structure[file_stem]
        # Heading text is the path-safe slug so the index link and the rendered id
        # agree even for a dotted stem; the true file name stays on the next line.
        body += [f"## {_path_slug(file_stem)}", "", f"Source file `{file_stem}.yml`.", ""]
        for group_name in sorted(groups):
            for rule in groups[group_name]:
                body += _rule_section(rule, group_name)
    page = {
        "path": catalogue_path(source_key),
        "title": f"{source_key} meter list",
        "nav_path": f"Observe / Meter list / {source_key}",
        "content": "\n".join(body).rstrip() + "\n",
    }
    if page["content"].count("{") != page["content"].count("}"):
        # Only an unbalanced PromQL expression can reach here (prose is
        # entity-escaped). Fail loudly rather than let the transform blame itself.
...
def rule_attributes(file_stem: str, group_name: str, rule: dict[str, Any], source_key: str = "hub2.prometheus") -> dict[str, Any]:
    attributes = {
        ...
        "source_page_path": catalogue_path(source_key),
        "group": group_name,
...
def main(argv: list[str] | None = None) -> int:
    ...
    catalogue = catalogue_path(source_key)
    for path in stale_paths:
        ...
        if current is not None and current.get("status") != "archived":
            ...
            # A retired generated path keeps resolving: inbound links and bookmarks
            # land on the catalogue. Redirect after archive — a redirect source
            # may never be an active page.
            operations.append(("ADD_REDIRECT", None, None, {"path": path, "from_path": path, "to_path": catalogue}))
    ...
    for operation_type, resource_id, revision, page in operations:
        ...
        elif operation_type == "ADD_REDIRECT":
            request["payload"] = {"from_path": page["from_path"], "to_path": page["to_path"]}
        if resource_id:
