...
def render_pages(source_key: str, structure: dict[str, Any], structure_hash: str) -> list[dict[str, str]]:
    ...
    by the publishing change, without aliases."""
    stamp = (
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Retired generated paths are archived WITHOUT an alias. Corpus redirect policy:
    # known inbound links are rewritten before the change, validation surfaces any
    # hidden dependency, and a redirect is added only where a continuing
    # compatibility requirement is demonstrated — never by default.
