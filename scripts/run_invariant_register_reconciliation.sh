...
lock_file="${DOCPLANE_INVARIANT_REGISTER_LOCK_FILE:-/run/lock/docplane-invariant-register.lock}"
metrics_file="${DOCPLANE_INVARIANT_REGISTER_METRICS_FILE:-/var/lib/node_exporter/textfile_collector/docplane_invariant_register.prom}"
...
flock -n -E "$FLOCK_CONFLICT_EXIT" "$lock_file" \
  python3 "$repository_root/scripts/invariant_register.py" "$@"
reconcile_status=$?
