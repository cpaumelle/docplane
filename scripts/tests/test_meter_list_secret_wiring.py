"""CANARY-1 consumer wiring: meter_list.py must resolve its bearer through the SECRETS-V3
...
TARGET = SCRIPTS / "meter_list.py"
TOKEN_VAR = "DOCPLANE_METER_LIST_TOKEN"
...
def test_consumer_prefers_the_file_over_a_stale_environment_value(tmp_path, monkeypatch):
    ...
    carries a legacy bearer while the runtime file is the intended source. Today the deployed
    /etc/charliehub/docplane-meter-list.env still delivers the plaintext value."""
