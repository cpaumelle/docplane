"""Append-only audit and integration event API for contributors."""
...
def ingest_events(request: EventBatch, principal: Principal = Depends(require_contributor)) -> EventReceipt:
    event_ids: list[str] = []
    accepted = duplicate = last_sequence = 0
    producer = f"{principal.principal_id}:{request.producer_id}"
    ...
    return EventReceipt(
        ...
        duplicate=duplicate,
        event_ids=event_ids,
        last_cursor=encode_cursor(last_sequence, principal_id=principal.principal_id) if last_sequence else None,
    )
