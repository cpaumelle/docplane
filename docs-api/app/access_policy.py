...
`private_fabric` treats reachability of the fabric-routed service as admission and
allows short-lived, individually attributable AGENT credentials to be minted over
HTTP. Bearer authentication remains mandatory for every protected API operation.
"""
...
def current_access_policy() -> AccessPolicy:
    ...
    if profile not in _ACCESS_PROFILES:
        raise RuntimeError(
            "DOCPLANE_ACCESS_PROFILE must be one of: " + ", ".join(sorted(_ACCESS_PROFILES))
        )
    ttl = _bounded_int(
        "DOCPLANE_SELF_ISSUE_TTL_SECONDS",
        _MAX_SELF_ISSUE_TTL_SECONDS,
        minimum=300,
        maximum=_MAX_SELF_ISSUE_TTL_SECONDS,
    )
    return AccessPolicy(
        profile=profile,
        ...
        source_limit_per_hour=_bounded_int(
            "DOCPLANE_SELF_ISSUE_SOURCE_LIMIT_PER_HOUR", 10, minimum=1, maximum=1000
        ),
        global_limit_per_hour=_bounded_int(
            "DOCPLANE_SELF_ISSUE_GLOBAL_LIMIT_PER_HOUR", 120, minimum=1, maximum=10000
        ),
    )
